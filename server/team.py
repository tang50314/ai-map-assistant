...
async def main():
    ...
    for query in test_queries:
        try:
            flush_print(f"\n{'='*60}")
            flush_print(f"开始测试查询: {query}")
            flush_print(f"{'='*60}")
            
            result = await process_map_query(query)
            flush_print(f"\n🎯 最终结果:\n{result}")
        except Exception as e:
            flush_print(f"❌ 测试查询 '{query}' 失败: {str(e)}")
        await asyncio.sleep(2)  # 避免过于频繁的请求

# ===================================================================
# 新增：三种专用处理函数（流式返回，供 FastAPI 使用）
# 完全复用您原有的 create_agents / create_mcp_workbench / process_messages 结构
# ===================================================================
...
async def process_general_query(user_query: str, conversation_id: int, db: Session) -> AsyncGenerator[Dict, None]:
    ...
    agent = AssistantAgent(
        ...
        description="通用对话助手",
        model_client=model_client,
        system_message="你是一个友好的AI助手，回答用户日常问题。用简洁、清晰、友好的语言回复。不要使用工具。"
    )
    
    termination = TextMentionTermination("TERMINATE")
    team = SelectorGroupChat([agent], model_client=model_client, termination_condition=termination)
    
    async for msg in team.run_stream(task=user_query):
        if isinstance(msg, TextMessage) and msg.source == "GeneralAgent":
            flush_print(f"🗣️ GeneralAgent: {msg.content}")
...
async def process_route_query(user_query: str, conversation_id: int, db: Session) -> AsyncGenerator[Dict, None]:
    ...
    try:
        ...
        planning = AssistantAgent(
            ...
            model_client=model_client,
            workbench=workbench,
            system_message="分析起点终点，规划最优驾车/步行/公交路线，考虑时间、距离、拥堵。"
        )
        tool = AssistantAgent(
            name="MapTool",
            description="调用地图工具",
        ...
        result = AssistantAgent(
            ...
            model_client=model_client,
            workbench=workbench,
            system_message="整理为 Markdown 路线卡：起点→终点、步数、时间、导航提示。"
        )
        end = AssistantAgent(
            name="EndAgent",
            description="结束",
            model_client=model_client,
            workbench=workbench,
            system_message="输出 TERMINATE"
        )

        team = SelectorGroupChat(
            [planning, tool, result, end],
            model_client=model_client,
            termination_condition=TextMentionTermination("TERMINATE")
        )

        async for msg in team.run_stream(task=f"路线规划：{user_query}"):
